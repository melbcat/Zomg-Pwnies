...
;;
;; Bindshell type shellcode. Focus on readability, so not minimal in any way.
;;
;; The general idea is to
;;  1) Listen for connections on a specified port.
;;  2) When a connection is accepted bind STD_IN and STD_OUT to corrosponding
;;     socket.
;;  3) Spawn a shell. The shell will now use your socket as STD_IN and STD_OUT
;;     You can also bind STD_ERR if you want to.
;;
;; Section 2 of the man pages describe system calls, so to get startet you might
...
;; But don't have that luxury, so be carefull to make the convertion by hand if
;; necessary (I bet you are on a little endian system, so it probably is).
;;
